...
REGISTRY_PATH = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "..", "config", "adapter_registry.json")
)
...
PROVIDER_OPTIONS = {
    "1": {
        "key": "ishares",
        "label": "iShares",
        "keywords": ["ishares", "blackrock", "ishs"],
    },
    "2": {
        "key": "amundi",
        "label": "Amundi / Lyxor",
        "keywords": ["amundi", "lyxor", "lyx", "multi units"],
    },
    "3": {
        "key": "xtrackers",
        "label": "Xtrackers (DWS)",
        "keywords": ["xtrackers", "db x-trackers", "dws", "x(ie)"],
    },
...
def validate_selection(choice_key, etf_name):
    ...
    provider_info = next(
        (p for k, p in PROVIDER_OPTIONS.items() if p["key"] == choice_key), None
    )
...
def update_registry_interactive(trades_df):
    ...
    if "ISIN" not in trades_df.columns or "NAME" not in trades_df.columns:
        logger.warning(
            "Trades DataFrame missing ISIN or NAME columns. Skipping registry update."
        )
        return
                ...
                if choice in PROVIDER_OPTIONS:
                    ...
                    if validate_selection(selected_key, name):
                        if selected_key != "skip":
                            ...
                            new_entries = True
                            print(
                                f"⏭️  Marked {isin} as 'ignore'. You won't be asked again."
                            )
                        break
                else:
