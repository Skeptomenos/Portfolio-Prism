...
def find_problematic_entries(cache):
    ...
    for key, data in cache.items():
        if (data.get('isin') == 'N/A' and 
            data.get('sector') != 'Unknown' and 
            data.get('geography') != 'Unknown'):
...
def update_asset_universe(resolved_securities: list):
    ...
    if os.path.exists(ASSET_UNIVERSE_PATH):
        ...
        df = pd.DataFrame(columns=['ISIN', 'TR_Ticker', 'Yahoo_Ticker', 'Name', 'Provider', 'Asset_Class'])
