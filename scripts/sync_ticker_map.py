...
def validate_map():
    ...
    if orphaned:
        warnings.append(f"⚠️  {len(orphaned)} orphaned ISINs (in map but not in universe)")
        if len(orphaned) <= 5:
    ...
    for _, row in universe_df.iterrows():
        ...
        if pd.notna(universe_ticker) and universe_ticker != '-':
            if isin in ticker_map and ticker_map[isin] != universe_ticker:
                inconsistencies.append(f"     {isin}: map={ticker_map[isin]}, universe={universe_ticker}")
    ...
    if missing:
        warnings.append(f"⚠️  {len(missing)} ISINs in universe but not in map (run sync to add)")
