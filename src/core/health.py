...
class PipelineHealth:
    ...
    def record_etf_stat(self, ticker: str, holdings_count: int, weight_sum: float, status: str = "OK"):
        ...
        if weight_sum < 1.0 and status == "OK": # Assuming weight is percentage 0-100 or 0-1? usually 0-100 in this project
    ...
    def generate_report(self) -> str:
        ...
        if not self.failures:
            ...
            for _, row in failures_df.iterrows():
                ...
                md.append(f"| {icon} {row['severity']} | `{row['item']}` | {row['error']} | **{row['fix']}** |")
        ...
        for etf in self.etf_stats:
            ...
            md.append(f"| {etf['ticker']} | {etf['holdings_count']} | {etf['weight_sum']:.2f}% | {status_icon} {etf['status']} |")
