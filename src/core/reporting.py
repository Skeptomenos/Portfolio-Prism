...
def generate_report(input_filepath: str = "outputs/true_exposure_report.csv"):
    ...
    try:
        ...
        logger.info(
            f"  - Successfully loaded exposure report with {len(exposure_df)} entries."
        )

        # --- FIX: Filter out rows with invalid ISINs before enrichment ---
        exposure_df.dropna(subset=["isin"], inplace=True)
        exposure_df = exposure_df[
            exposure_df["isin"].apply(lambda x: isinstance(x, str))
        ]

        # Get a unique list of securities (ISINs) for enrichment
        securities_to_enrich = (
            exposure_df[["isin"]].drop_duplicates().to_dict("records")
        )
        ...
        if not enriched_data:
            logger.warning(
                "  - Enrichment returned no data. Skipping sector/geography analysis."
            )
            return
        ...
        if missing_value_count > 0:
            logger.warning(
                f"  - ⚠️  {missing_value_count} assets have missing value data (Price not found). They are excluded from total portfolio value."
            )
        ...
        if "asset_class" in final_df.columns:
            ...
            final_df.loc[cash_mask, "sector"] = final_df.loc[
                cash_mask, "sector"
            ].fillna("Cash & Equivalents")
            final_df.loc[cash_mask, "geography"] = final_df.loc[
                cash_mask, "geography"
            ].fillna("Global")

            # Derivatives
            deriv_mask = final_df["asset_class"] == "Derivative"
            final_df.loc[deriv_mask, "sector"] = final_df.loc[
                deriv_mask, "sector"
            ].fillna("Derivatives")
            final_df.loc[deriv_mask, "geography"] = final_df.loc[
                deriv_mask, "geography"
            ].fillna("Global")
        ...
        # Save Enriched Report (for Visualization)
        final_df.to_csv('outputs/enriched_exposure_report.csv', index=False)
        logger.info("  - Enriched exposure report saved to outputs/enriched_exposure_report.csv")
        ...
        sector_exposure = (
            final_df.groupby("sector")["total_exposure"].sum().reset_index()
        )
        total_portfolio_value = final_df["total_exposure"].sum()
        if total_portfolio_value > 0:
            sector_exposure["portfolio_percentage"] = (
                sector_exposure["total_exposure"] / total_portfolio_value
            ) * 100
        else:
        ...
        geography_exposure = (
            final_df.groupby("geography")["total_exposure"].sum().reset_index()
        )
        if total_portfolio_value > 0:
            geography_exposure["portfolio_percentage"] = (
                geography_exposure["total_exposure"] / total_portfolio_value
            ) * 100
        else:
