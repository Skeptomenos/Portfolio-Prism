...
def load_portfolio_state():
    ...
    if os.path.exists(UNIVERSE_PATH) and os.path.exists(HOLDINGS_PATH):
        logger.info("Loading portfolio from Relational Database (Universe + Holdings)...")
        ...
        if not unmapped.empty:
            logger.warning(f"{len(unmapped)} assets in Holdings could not be mapped to Universe (Check ISINs).")
        ...
        if 'ISIN' not in df.columns: df['ISIN'] = df['Ticker']
        if 'Asset_Class' not in df.columns: df['Asset_Class'] = 'Stock'
    else:
