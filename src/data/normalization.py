...
ASSET_NAMES_CACHE_PATH = os.path.join(os.path.dirname(__file__), '..', '..', 'config', 'asset_names.json')
# Path to the ticker map
TICKER_MAP_PATH = os.path.join(os.path.dirname(__file__), '..', '..', 'config', 'ticker_map.json')
...
def _fetch_name_yfinance(isin):
    ...
    try:
        ...
        logger.warning(f"YFinance name lookup failed for {isin} (Ticker: {ticker_symbol}): {e}")
        # pass
    return None
