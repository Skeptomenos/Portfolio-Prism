...
ISHARES_CONFIG_PATH = os.path.join(os.path.dirname(__file__), '..', '..', 'config', 'ishares_config.json')
...
class ISharesAdapter:
    ...
    def _prompt_for_product_id(self, isin: str) -> str:
        ...
        print("   Please visit the iShares website, find the ETF page, and look at the URL.")
        print("   Example URL: .../produkte/251882/ishares-msci-world-ucits-etf-acc-fund")
    ...
    def fetch_holdings(self, isin: str) -> pd.DataFrame:
        ...
        try:
            ...
            def get_yahoo_suffix(row):
                ...
                if location == 'Vereinigte Staaten' or 'NASDAQ' in exchange or 'New York' in exchange:
                    return ""
            ...
            logger.error(f"Network request failed for {isin} (ID: {product_id}). Details: {e}")
            return pd.DataFrame()
        except Exception as e:
            logger.error(f"An unexpected error occurred in ISharesAdapter for {isin}: {e}")
            return pd.DataFrame()
