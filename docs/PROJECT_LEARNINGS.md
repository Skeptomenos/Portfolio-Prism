...
- **Strategy: Global vs. Regional Pricing APIs (Phase 5)**
  ...
  - **Case Study: Optimizing CPU-Bound PDF Parsing (Phase 7)**
    - **Problem:** Parsing 200+ page PDFs with `pdfplumber` is extremely slow (single-core CPU bound), taking 10+ minutes for a full run.
    - **Solution:** 
      1.  **Multiprocessing:** Implemented `multiprocessing.Pool` to parallelize parsing at the page level. *Constraint:* `pdfplumber` objects are not pickleable, so the worker function must accept a file path and page index, opening the file inside the worker.
      2.  **Incremental Loading:** Implemented a `processed_files` table (SHA256 hash) and a `trades` table (UNIQUE constraint) in SQLite. The parser now skips known files entirely.
    - **Concurrency Note:** SQLite disallows multiple writers. The multiprocessing workers return data to the main process, which performs a single bulk `INSERT` to avoid `database is locked` errors.
    - **Result:** Parsing a 200-page file dropped from ~10 mins to <2 mins (initial) and <1 sec (subsequent).
    - **Principle:** Isolate CPU-heavy logic from I/O logic. Use hashing for file-level deduplication and database constraints for row-level deduplication (handling cumulative/overlapping reports).

  - **Case Study: Handling Malformed/Strict XML in Excel Files (Phase 7)**
    - **Problem:** Amundi's export files (`.xlsx`) contain invalid XML (e.g., bad namespace or stylesheet references), causing standard Python tools like `openpyxl` (and thus `pandas`) to crash with `Unable to read workbook`.
    - **Solution:** We identified `python-calamine` (Rust-based) as a robust alternative. It tolerates malformed OOXML structures.
    - **Implementation:** The `AmundiAdapter` now uses a tiered approach: it tries the standard engine first, and if that fails, it dynamically falls back to `engine='calamine'`.
    - **Principle:** **Adaptive Robustness.** Do not rely on a single parser implementation. When data quality is out of your control (upstream), build redundancy into the parsing layer using libraries with different philosophies (Strict XML vs. Data Dump).

  - **Case Study: Yahoo Finance Ticker Constraints (Phase 8)**
